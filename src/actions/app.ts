...
export const navigate: ActionCreator<ThunkResult> = (path: string) => dispatch => {
  ...
  const page = path === '/' ? 'web-bluetooth' : path.slice(1);
...
const loadPage: ActionCreator<ThunkResult> = (page: string) => dispatch => {
  switch (page) {
    case 'web-bluetooth':
      import('../components/my-web-bluetooth').then(_module => {
        // Put code in here that you want to run every time when
        // navigating to web-bluetooth after my-web-bluetooth.js is loaded.
      });
      break;
    case 'web-usb':
      import('../components/my-web-usb.js');
      break;
    case 'web-nfc':
      import('../components/my-web-nfc.js');
      break;
    default:
      page = 'view404';
