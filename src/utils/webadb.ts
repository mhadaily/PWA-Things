...
(function() {
  ...
  Adb.open = function(transport: any) {
  ...
  Adb.WebUSB.Transport = function(device: any) {
    this.device = device;
  ...
  Adb.WebUSB.Transport.open = function() {
    ...
    return (<any>navigator).usb
      .requestDevice({ filters: filters })
      .then((device: any) => device.open().then(() => new Adb.WebUSB.Transport(device)));
  };
  ...
  Adb.WebUSB.Transport.prototype.send = function(ep: any, data: any) {
    if (Adb.Opt.dump) hexdump(new DataView(data), '' + ep + '==> ');

    return this.device.transferOut(ep, data);
  };

  Adb.WebUSB.Transport.prototype.receive = function(ep: any, len: any) {
    return this.device.transferIn(ep, len).then((response: any) => {
  ...
  Adb.WebUSB.Transport.prototype.find = function(filter: any) {
  ...
  Adb.WebUSB.Transport.prototype.getDevice = function(filter: any) {
  ...
  Adb.WebUSB.Transport.prototype.connectAdb = function(banner: any, auth_user_notify: any = null) {
    ...
    return this.getDevice({ classCode: 255, subclassCode: 66, protocolCode: 1 })
      .then((match: any) => new Adb.WebUSB.Device(this, match))
      .then((adb: any) =>
        m
          .send_receive(adb)
          .then(function do_auth_response(response: any) {
            if (response.cmd != 'AUTH' || response.arg0 != AUTH_TOKEN) return response;
          ...
          .then((response: any) => {
            if (response.cmd != 'CNXN') throw new Error("Failed to connect with '" + banner + "'");
            console.log('version', response.arg0);
            if (response.arg0 != VERSION && response.arg0 != VERSION_NO_CHECKSUM)
              throw new Error(
                'Version mismatch: ' + response.arg0 + ' (expected: ' + VERSION + ' or ' + VERSION_NO_CHECKSUM + ')'
              );
            if (Adb.Opt.debug) console.log("Connected with '" + banner + "', max_payload: " + response.arg1);
            adb.max_payload = response.arg1;
            if (response.arg0 == VERSION_NO_CHECKSUM) Adb.Opt.use_checksum = false;
            return adb;
          })
      );
  };
  ...
  Adb.WebUSB.Transport.prototype.connectFastboot = function() {
    return this.getDevice({ classCode: 255, subclassCode: 66, protocolCode: 3 })
      .then((match: any) => new Fastboot.WebUSB.Device(this, match))
      .then((fastboot: any) =>
        fastboot.send('getvar:max-download-size').then(() =>
          fastboot.receive().then((response: any) => {
            let cmd = decode_cmd(response.getUint32(0, true));
            if (cmd == 'FAIL') throw new Error('Unable to open Fastboot');

            fastboot.get_cmd = (r: any) => decode_cmd(r.getUint32(0, true));
            fastboot.get_payload = (r: any) => r.buffer.slice(4);
            return fastboot;
          })
        )
      );
  ...
  Adb.WebUSB.Device = function(transport: any, match: any) {
  ...
  Adb.WebUSB.Device.prototype.open = function(service: any) {
    return Adb.Stream.open(this, service);
  };

  Adb.WebUSB.Device.prototype.shell = function(command: any) {
    return Adb.Stream.open(this, 'shell:' + command);
  };

  Adb.WebUSB.Device.prototype.tcpip = function(port: any) {
  ...
  Adb.WebUSB.Device.prototype.reboot = function(command: any = '') {
    return Adb.Stream.open(this, 'reboot:' + command);
  };

  Adb.WebUSB.Device.prototype.send = function(data: any) {
  ...
  Adb.WebUSB.Device.prototype.receive = function(len: any) {
  ...
  Fastboot.WebUSB.Device = function(transport: any, match: any) {
    this.transport = transport;
  ...
  Fastboot.WebUSB.Device.prototype.send = function(data: any) {
    if (typeof data === 'string') {
      let encoder = new TextEncoder();
      let string_data = data;
      data = encoder.encode(string_data).buffer;
  ...
  Adb.Message = function(cmd: any, arg0: any, arg1: any, data: any = null) {
  ...
  Adb.Message.checksum = function(data_view: any) {
    let sum = 0;
  ...
  Adb.Message.send = function(device: any, message: any) {
    ...
    if (message.data != null) {
      ...
      len = data.byteLength;
      if (Adb.Opt.use_checksum) checksum = Adb.Message.checksum(new DataView(data));
  ...
  Adb.Message.receive = function(device: any) {
    return device
      ...
      .then((response: any) => {
        ...
        return device.receive(len).then((data: any) => {
          if (Adb.Opt.use_checksum && Adb.Message.checksum(data) != check) throw new Error('checksum mismatch');
  ...
  Adb.Message.prototype.send = function(device: any) {
    return Adb.Message.send(device, this);
  };

  Adb.Message.prototype.send_receive = function(device: any) {
    return this.send(device).then(() => Adb.Message.receive(device));
  };

  Adb.SyncFrame = function(cmd: any, length: any = 0, data: any = null) {
    if (cmd.length != 4) throw new Error("Invalid command: '" + cmd + "'");
  ...
  Adb.SyncFrame.send = function(stream: any, frame: any) {
  ...
  Adb.SyncFrame.receive = function(stream: any) {
    return stream.receive().then((response: any) => {
  ...
  Adb.SyncFrame.prototype.send = function(stream: any) {
    return Adb.SyncFrame.send(stream, this);
  };

  Adb.SyncFrame.prototype.send_receive = function(stream: any) {
    return Adb.SyncFrame.send(stream, this).then(() => Adb.SyncFrame.receive(stream));
  };

  Adb.Stream = function(device: any, service: any, local_id: any, remote_id: any) {
    this.device = device;
  ...
  Adb.Stream.open = function(device: any, service: any) {
    let local_id = next_id++;
    let remote_id = 0;

    let m = new Adb.Message('OPEN', local_id, remote_id, '' + service + '\0');
    return m.send_receive(device).then(function do_response(response: any) {
      if (response.arg1 != local_id) return Adb.Message.receive(device).then(do_response);
  ...
  Adb.Stream.prototype.send = function(cmd: any, data: any = null) {
  ...
  Adb.Stream.prototype.receive = function() {
    return Adb.Message.receive(this.device).then((response: any) => {
      // remote's prospective of local_id/remote_id is reversed
      if (response.arg0 != 0 && response.arg0 != this.remote_id)
        throw new Error(
          'Incorrect arg0: 0x' + toHex32(response.arg0) + ' (expected 0x' + toHex32(this.remote_id) + ')'
        );
  ...
  Adb.Stream.prototype.send_receive = function(cmd: any, data: any = null) {
  ...
  Adb.Stream.prototype.stat = function(filename: any) {
    ...
    return frame
      ...
      .then(check_ok('STAT failed on ' + filename))
      .then(() => {
        return this.receive().then((response: any) => this.send('OKAY').then(() => response.data));
      })
      .then((response: any) => {
  ...
  Adb.Stream.prototype.pull = function(filename: any) {
    ...
    return frame
      ...
      .then((response: any) => {
        ...
        if (response.data.byteLength == len + 8) {
          ...
          if (decode_cmd(cmd) != 'DONE' || zero != 0) throw new Error('PULL DONE failed on ' + filename);
  ...
  Adb.Stream.prototype.push_start = function(filename: any, mode: any) {
  ...
  Adb.Stream.prototype.push_data = function(data: any) {
    if (typeof data === 'string') {
      let encoder = new TextEncoder();
      let string_data = data;
      data = encoder.encode(string_data).buffer;
  ...
  Adb.Stream.prototype.push = function(file: any, filename: any, mode: any, on_progress: any = null) {
    ...
    // read the whole file
    return read_blob(file).then((data: any) =>
  ...
  function check_cmd(cmd: any, err_msg: any) {
  ...
  function check_ok(err_msg: any) {
    return check_cmd('OKAY', err_msg);
  }

  function paddit(text: any, width: any, padding: any) {
  ...
  function get_ep_num(endpoints: any, dir: any, type: any = 'bulk') {
    let e, ep;
    for (e in endpoints) if (((ep = endpoints[e]), ep.direction == dir && ep.type == type)) return ep.endpointNumber;
    if (Adb.Opt.debug) console.log(endpoints);
    throw new Error('Cannot find ' + dir + ' endpoint');
  }

  function encode_cmd(cmd: any) {
    let encoder = new TextEncoder();
  ...
  function do_auth(adb: any, keys: any, key_idx: any, token: any, do_auth_response: any, auth_user_notify: any) {
    ...
    if (key_idx < keys.length) {
      ...
      return (
        seq
          // @ts-ignore
          .then(() => crypto.subtle.sign({ name: 'RSASSA-PKCS1-v1_5' }, key.privateKey, token))
          .then(signed => {
            if (Adb.Opt.dump) console.log('-----BEGIN SIGNATURE-----\n' + toB64(signed) + '\n-----END SIGNATURE-----');
    ...
    if (seq === null) {
      if (Adb.Opt.debug) console.log('generating key ' + key_idx + ' (' + Adb.Opt.key_size + ' bits)...');

      seq = generate_key();
      dirty = true;
    }
    // @ts-ignore
    return seq.then((key: any) => {
      return crypto.subtle
  ...
  function promisify(request: any, onsuccess = 'onsuccess', onerror = 'onerror') {
    return new Promise(function(resolve, reject) {
